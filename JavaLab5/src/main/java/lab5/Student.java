...
public class Student implements Comparable<Student> {
    ...
    public @Pattern(regexp = "[A-Z][a-z]{1,32}", message = "Name must consists only from letter and first letter must be UpperCase") String getName() {
        return name;
    }
    ...
    public @PastOrPresent(message = "Must be a date in the past or in the present") LocalDate getBirthday(){
        return birthday;
    }
    ...
    public static void main(String args[]){
        try{
            Student st = new Student.Builder()
                    .setName("mariya")
                    .setSurname("did453")
                    .setBirthday(LocalDate.of(2022, 12, 29))
                    .build();
